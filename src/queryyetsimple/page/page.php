...
class Page333333 {
    ...
    public static function RUN($nCount = 0, $nSize = 1, $sUrl = '', $sParameter = '', $nPage = null, $bDefaultIns = true) {
        ...
        $oPage = new static ( $nCount, $nSize, $sUrl, $sParameter, $nPage ); // 创建一个分页对象
    ...
    public function P($arrOption = array(), $sPagename = 'page') {
        ...
        if (! empty ( $this->_sUrl )) {
            if (strpos ( $this->_sUrl, '@' ) === 0) {
                $sUrl = Q::U ( ltrim ( $this->_sUrl, '@' ), false === strpos ( $this->_sUrl, '{page}' ) ? array (
                        $this->_sPagename => '{page}' 
                ) : array () );
            } else {
