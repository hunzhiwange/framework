...
namespace Leevel\Level\Process;
...
class HotOverload extends Process
{
    ...
    {
        $this->option = $option;
        $this->delayCount = (int) $this->option->get('level\\hotoverload_delay_count', 2);
        $this->timeInterval = (int) $this->option->get('level\\hotoverload_time_interval', 20);
    }
    ...
    {
        $files = [];
        foreach ((array) $this->option->get('level\\hotoverload_watch', []) as $dir) {
            if (is_file($dir)) {
                $files[] = $dir;
